...
if(navigator.mediaDevices && navigator.mediaDevices.getUserMedia) { 
    // Не включаем аудио опцией `{ audio: true }` поскольку сейчас мы работаем только с изображениями
		...
		navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 640, max: 1280}, height: {ideal: 360, max: 720 }, frameRate: 10} }).then(function(stream) {
		...
		.catch(function(err) {
			...
			if (getCookie("id")!=="st110") {alert("Для тестирования необходимо включить камеру"); document.location.href = "../../../welcome.php";}
...
//---------------------
    ...
    //  };
	 ...
	 async function reloadPage() { 
		myVar = 'nameUser='+ encodeURIComponent(getCookie("id_xtvs"));   
		...
		let attempt=0;
			...
			// 3. Настройка запроса
			request.open('POST','../../../application/save_attempt.php',true);
    ...
    // 5. Отправка запроса на сервер. В качестве параметра указываем данные, которые необходимо передать (необходимо для POST)
			...
			request.onreadystatechange = await function() { // Ждём ответа от сервера
				if (request.readyState == 4) { // Ответ пришёл
				  if(request.status == 200) { attempt = request.response; 
					prints(attempt);
...
function prints(a)	{ 
	if (a==0)
{ alert("Возможна только одна попытка прохождения тестирования в день"); 
window.location.href="../../../welcome.php"; } }
	
if (courseGo==305||courseGo==303) { popupVideo(); let confirmName = confirm("вы подтверждаете свою учётную запись?\nВы - "+getCookie("name_xtvr")+"?\nВНИМАНИЕ! В рамках тестирования ведется аудио-видеофиксация процесса его прохождения."
+ "\nВ случае выявления нарушений условий прохождения тестирования итоговые результаты будут аннулированы!"+
"\nЗапрещается использование вспомогательных средств, прохождение тестирования с использованием чужого табельного номера, пользоваться сторонней помощью.");
if(!confirmName){ window.location.href="https://training.aeromash.ru/xtvs/"; }}
...
// Получаем доступ к камере 
/*if(navigator.mediaDevices && navigator.mediaDevices.getUserMedia&&noVideo&&courseGo!==301) { 
    // Не включаем аудио опцией `{ audio: true }` поскольку сейчас мы работаем только с изображениями
    
		navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 640, max: 1280}, height: {ideal: 360, max: 720 }, frameRate: 60} }).then(function(stream) {
...
navigator.mediaDevices.getUserMedia({ audio: true, video: true})
    .then(stream => {
        ...
        let myclick=document.getElementById('wwrapper');
		myclick.addEventListener("click",function(event) {  
			
            if((viewSlide*1)>10&&(viewSlide*1)<19&&recordVideo&&videoSave==0){ videoSave=1; mediaRecorder.stop(); mediaRecorder.start();
				}});
        

				myclick.addEventListener("click",function(event) { 
            if((viewSlide*1)>20&&(viewSlide*1)<29&&recordVideo){if(videoSave==1||videoSave==0)  { videoSave=2;mediaRecorder.stop(); mediaRecorder.start();}}});
        

			myclick.addEventListener("click",function(event) { 
            if((viewSlide*1)>34&&(viewSlide*1)<43&&recordVideo){ if(videoSave==2||videoSave==1||videoSave==0) {videoSave=3;mediaRecorder.stop(); mediaRecorder.start();}}});

			myclick.addEventListener("click",function(event) { 
				if((viewSlide*1)>48&&recordVideo){ mediaRecorder.stop(); recordVideo = false;}});
...
let objectfind;
if (courseGo==305||courseGo==303) {objectfind = ["вы тестируетесь по программе инспектора ДДПП"]}
else {objectfind = ["вы тренируетесь по программе инспектора ДДПП"]}

let superTimer=0;
let viewobjecttext = document.getElementById("viewobjecttext");
let viewscope = document.getElementById("scope");
if (courseGo==305||courseGo==303) {viewscope.innerHTML = "&#128518;";}
	else {viewscope.innerHTML = 0+ "%" ;} //viewscope.innerHTML="&#128518";//0; "&#128518;"
...
viewobjectfind.innerHTML = getCookie("name_xtvr")+"<br>"+objectfind[0]; 
...
shuffle(); // перемешиваем картинки

if(courseGo==303){cutOff(allSliders);} else {allSliders=20; cutOff(allSliders);} // количество картинок в тесте
//console.log (pathAria); // ПОДСКАЗКА УБРАТЬ

for(let i=0; i<slideAll; i++) {

let folder;	
let b = randoms(0,1); // определяем есть закладка или нет
...
{
// если закладка есть то, определяем есть ли вторая проекция
if(pathAria_1[i]==undefined){rotateArr[i]=false;} else {rotateArr[i]=true;}
if(pathAria2[i]==undefined){folder = 1;} else {folder = randoms(1,2); } // определяем закладку из какой папки вкладываем 
...
btnNext.addEventListener("click", function () {  pauseGO=1; region77=0;  
	stopzooom();  //rotate(0);  
	
	
	
	//if (viewSlide > 0)	{document.getElementById('slide' + (viewSlide+0)).setAttribute('src',  pathname[viewSlide]  + files[(viewSlide+0)] + '+'+dop + '.jpg'); }
...
function click_handler() { 
	...
	{viewSlide++; if(!rotateArr[viewSlide]){btnSecond.style.display='none';}else{btnSecond.style.display='flex';} 
	timeGo=0; nameSlide++;   	sumClicksOneImage=0;
	//pathname[viewSlide]=image_rti; pauseGO=0;				
viewSlideShow.innerHTML ="осталось " + (allSliders - 1 - viewSlide) 
...
viewSlideShow.innerHTML = viewSlide; } }
    ...
    slider.style.left = (-viewSlide * viewport-timeGo) + "px";
		...
		btnSensetiv.addEventListener("click", function () {dop = "S"; region77=0; 
		dopView();
		region77==1;} );
		
		btnHi.addEventListener("click", function () {dop = "H"; region77=0; 
		dopView();
		region77==1;} );
		
		btnNonOrganic.addEventListener("click", function () {dop = "OS"; region77=0; 
		dopView();
		region77==1;} );
		
		btnOrganic.addEventListener("click", function () {dop = "OO"; region77=0; 
		dopView();
		region77==1;} );
		
		btnBlackwhite.addEventListener("click", function () {dop = "B"; region77=0;  
		dopView();
		region77==1;} );
		
		
		
		btnInverse.addEventListener("click", function () {dop = "NE"; region77=0; 
		dopView(); 
		region77==1;} );
		
		btnExit.addEventListener("click", function () {dop = "NO"; region77=0;   stopzooom(); dopView();
		...
		for(let i=viewSlide; i<(slideAll); i++) {
		
		document.getElementById('slide' + (i)).setAttribute('src',  pathname[i] + files[(i)] + '+' + dop + '.jpg'); 
		
		
		//document.getElementById('slide' + (viewSlide+1)).setAttribute('src',  pathname[viewSlide] + files[(viewSlide+1)] + '+' + dop + '.jpg'); 
		}
...
function superClick(x, y){  
	...
	if (courseGo==305||courseGo==303) {viewscope.innerHTML = "&#128518;";}
	else {viewscope.innerHTML = Math.round((scope/sumscopes)*100) + "%" ;}	// 	Math.round((scope/sumscopes)*100) + "%" ;			
...
function superClick1(x, y){
	...
	let liLast = document.createElement('div');
		...
		//console.log("1",liLast.style.left);
 ...
 $("#slide").click(function  (event) {
        //++clicks; 
		if (zooming){return false;} //pauseGO!==0||
...
newWin.document.write(viewobjectslider); qwert=0; sleep(100);  console.log('Taking a break...',viewobjectslider); } 
else { qwert=1;}} 
...
if(getCookie("id_xtvs")==null||getCookie("id_xtvs")==""){document.location='https://training.aeromash.ru/xtvs/';}
