...
navigator.mediaDevices.getUserMedia({ audio: true, video: true})
    .then(stream => {
		...
		document.addEventListener("click",function(event) {
            if(viewSlide==15&&recordVideo){mediaRecorder.stop(); recordVideo = false;}
        });
...
function click_handler() { 
	...
	{viewSlide++;timeGo=0; nameSlide++; pauseGO=0; 
...
viewobject.innerHTML = '<img src=' + image_object  + (0) + '.png width="250" alt=""> ';}}
  ...
  console.log('Taking a break...');
	...
	let courseGo = getCookie("courseGo"); 			
			...
			myVar = 'nameUser='+ encodeURIComponent(myVar)+" "+encodeURIComponent(timefortest)+" "+encodeURIComponent(scopess)+" "+encodeURIComponent(courseGo)+" "+encodeURIComponent(clicks)+" "+encodeURIComponent(secondsAll)+" "+encodeURIComponent(sumscopes/10);   
...
viewSlideShow.innerHTML = viewSlide; } }
    ...
    slider.style.left = (-viewSlide * viewport-timeGo) + "px";
		...
		dop = "NO"; region77=0; console.log(dop,region77,viewSlide); pauseGO=0;  //stopzooom(); 
		rotate(1);
		document.getElementById('slide' + (viewSlide+0)).setAttribute('src',  pathname[viewSlide] + files[(viewSlide+0)] + '+' + dop + '.jpg'); 
		//document.getElementById('slide' + (viewSlide+1)).setAttribute('src',  pathname + files[(viewSlide+1)] + '+' + dop + '.jpg'); region77==1;
		} );
