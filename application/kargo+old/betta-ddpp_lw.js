...
if(navigator.mediaDevices && navigator.mediaDevices.getUserMedia) { 
    // Не включаем аудио опцией `{ audio: true }` поскольку сейчас мы работаем только с изображениями
		...
		.catch(function(err) {
			...
			if (getCookie("id")!=="444") {alert("Для тестирования необходимо включить камеру"); document.location.href = "../../../welcome.php";}
...
viewSlideShow.innerHTML = viewSlide; } }
    ...
    slider.style.left = (-viewSlide * viewport-timeGo) + "px";
		...
		btnSensetiv.addEventListener("click", function () {dop = "S"; region77=0; pauseGO=1; 
		...
		btnHi.addEventListener("click", function () {dop = "H"; region77=0; pauseGO=1; 
		document.getElementById('slide' + (viewSlide)).setAttribute('src',  pathname[viewSlide] + files[(viewSlide+0)] + '+' + dop + '.jpg'); // 1=S чуствительность
		region77==1;} );
		
		btnNonOrganic.addEventListener("click", function () {dop = "OS"; region77=0; pauseGO=1; 
		document.getElementById('slide' + (viewSlide)).setAttribute('src',  pathname[viewSlide] + files[(viewSlide+0)] + '+' + dop + '.jpg'); 
		region77==1;} );
		
		btnOrganic.addEventListener("click", function () {dop = "OO"; region77=0; pauseGO=1; 
		document.getElementById('slide' + (viewSlide)).setAttribute('src',  pathname[viewSlide] + files[(viewSlide+0)] + '+' + dop + '.jpg'); 
		region77==1;} );
		
		btnBlackwhite.addEventListener("click", function () {dop = "B"; region77=0; pauseGO=1; 
		document.getElementById('slide' + (viewSlide+0)).setAttribute('src',  pathname[viewSlide] + files[(viewSlide+0)] + '+' + dop + '.jpg'); 
		region77==1;} );
		
		
		
		btnInverse.addEventListener("click", function () {dop = "NE"; region77=0; console.log(dop,region77,viewSlide); pauseGO=1; 
		document.getElementById('slide' + (viewSlide+0)).setAttribute('src',   pathname[viewSlide] + files[(viewSlide+0)] + '+' + dop + '.jpg'); 
		region77==1;} );
		
		btnExit.addEventListener("click", function () {dop = "NO"; region77=0; console.log(dop,region77,viewSlide); pauseGO=1;  stopzooom(); 
		//alert(pathname[viewSlide]); 
		document.getElementById('slide' + (viewSlide+0)).setAttribute('src',  pathname[viewSlide] + files[(viewSlide+0)] + '+' + dop + '.jpg'); 
		document.getElementById('slide' + (viewSlide+1)).setAttribute('src',  pathname[viewSlide] + files[(viewSlide+1)] + '+' + dop + '.jpg'); region77==1;
		} );
...
function superClick(x, y){  
	...
	let liLast = document.createElement('div');
		...
		console.log("2",liLast.style.left);
		console.log("2",event.offsetX);
		liLast.innerHTML = '&#x25CF;';
	slide.append(liLast);} }); } 
