...
let files = loadDoc(nowLesson, course);

//loadDoc собирает изображения из папки аргумента.
...
sessionStorage.setItem('regiomImg', '');
   ...
   clearInterval(timer); 
			...
			function demo2() {
			 ...
			 setTimeout(document.location.href = "otchet.html",200);
			 }
 ...
 let myVar = getCookie("id"); 
			...
			myVar = 'nameUser='+ encodeURIComponent(myVar)+" "+encodeURIComponent(timefortest)+" "+encodeURIComponent(scopess)+" "+encodeURIComponent(levelmva)+" "+encodeURIComponent(clicks);   
			...
			// 3. Настройка запроса
			request.open('POST','connect2_.php',true);
...
console.log('wqqqw', regiomImg);} else {viewobject.innerHTML = '<img src="'+ pathname + files[(viewSlide+0)] +'+NO.jpg"width="250" alt=""/>';
