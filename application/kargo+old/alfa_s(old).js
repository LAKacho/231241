...
var slideAll = pathAria.length; //window.storage.slideAll*1;
...
//console.log(i,files[i],things.length);
	...
	if (!changes){
	pauseGO=1; dop=0; region77=0;  // stopzooom(); 
	stopzooom(); console.log(changes);
	...
	console.log("GO");	
	if (viewSlide != slideAll){	document.getElementById('slide' + (viewSlide+1)).setAttribute('src', pathname  + files[(viewSlide+1)] + '+NO.jpg'); }} else {change();}});
...
function click_handler() { 
 ...
 viewobjectfind.innerHTML = getCookie("name_xtvr") + "<br>Вам необходимо: <br> Кликните левой кнопкой мыши на разные объекты. Выбранный объект выделяется рамкой, и появляется его описание." ; 
//если описание индивидуальное то objectfind[viewSlide]; 
...
/*viewobjectfind.innerHTML = "Кликните левой кнопкой мыши на разные объекты. Выбранный объект выделяется рамкой, и появляется его описание." ; 
//если описание индивидуальное то objectfind[viewSlide]; "id", "imageObject"*/
let photoBag='<img id="image_object" src=' + image_object  + files[(viewSlide+0)] + '.jpg width="250" alt=""> ';

viewobject.innerHTML = photoBag;
sessionStorage.setItem('regiomImg', '');
}}
...
viewSlideShow.innerHTML = viewSlide; } }
    ...
    slider.style.left = (-viewSlide * viewport-timeGo) + "px";
		...
		//return viewobjectslider='<img src='+image_object  + (0) +  '.png  alt="">'; 
		return dop;
		return pauseGO;
		return region77;
		return viewSlide;
		...
		btnExit.addEventListener("click", function () {dop = "NO"; region77=0; console.log(dop,region77,viewSlide); stopzooom(); 
 ...
 $("#slide").click(function  (event) {
  ...
  console.log(event);
	...
	//-------------------------засада----------------------					   
						   ...
						   let regionSlice=region;
						   
						   //console.log(regionSlice.slice(0, 2)*1);
						   
						   if(isNaN(regionSlice.slice(0, 2)*1)){
						   console.log("OK");
						   regiomImg =  pathname + files[slideClick]+'+NO_' + region.slice(0, -1).slice(1) + '.png';} 
						   else 
						   { console.log("NO");
							regiomImg =  pathname + files[slideClick]+'+NO_' + region.slice(0, -1).slice(2) + '.png';
							   }
						   
						   document.querySelector(selector).setAttribute('src', regiomImg);

							
  try {       let test='<img src='+ image_object  + region +'.jpg width="250" alt="" id="image_object">';    
        						   
				  viewobject.innerHTML = test;
				  sessionStorage.setItem('regiomImg', regiomImg);
						 
} catch (err) { console.log('<img src='+ image_object  + (0) +'.png width="250" alt="">');
	viewobject.innerHTML = '<img src='+ image_object  + region +'.png width="250" alt="">';
 console.log(err);
  // обработка ошибки
...
mimg.style.width=1003+ "px";
mimg.style.height=644+ "px";
...
console.log('wq1111', regiomImg); changes=true; return changes;} else {viewobject.innerHTML = '<img src="'+ pathname + files[(viewSlide+0)] +'+NO.jpg"width="250" alt=""/>';
console.log('wqqqw', "slide"); changes=true; return changes;}	
	} 
	catch (err) { let mimgs = document.getElementById('imageObject'); 
				  try {let mimgd = document.getElementById('imageObject').src;
			  setTimeout(() => mimgs.remove(), 10); pauseGO=tempPauseGO; 
			  viewobject.innerHTML = '<img src="' + mimgd +'"width="250" id="image_object" alt=""/>';  
	changes=false; return changes;} catch (err) {} }
	}
	
	 
	
	/*let b= sessionStorage.getItem('start');
    if (b===null) {sessionStorage.setItem('start',1);  }
	if (b===1) {sessionStorage.setItem('start', null);   }
	 alert(b); */
...
function loadDoc(lessons, course) {
  ...
  xhttp.open("POST", "../../../application/Read_ImgN3.php", false);
