 ...
 // Добавляем новые переменные для работы с камерой и распознаванием
var GlobalVar = false;
var noVideo = true;
...
message.id = 'faceDetectionMessage';
...
if(navigator.mediaDevices && navigator.mediaDevices.getUserMedia) { 
    message.textContent = 'Инициализация камеры...';
        ...
        const interval = setInterval(async () => {
            ...
            if (detection && detection.box) {
                ...
                if (dx < threshold && dy < threshold) {
                    ...
                    message.textContent = 'Лицо распознано';
                    resolve(true);
                } else {
                    message.textContent = 'Поместите лицо в центр кадра';
                }
            } else {
                message.textContent = 'Лицо не обнаружено';
            }
        }, 300);
    });
}
