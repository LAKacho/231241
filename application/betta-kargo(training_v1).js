...
if (courseGo==305) {popupVideo();} else {pauseGO = 1; noVideo=false;}
...
let objectfind=["Вам необходимо найти предметы заприщенные к провозу"]; 
if (courseGo==305 || courseGo==304) {objectfind= ["Вы проверяете груз с товарами народного потребления, Вам необходимо найти: оружие, боеприпасы и газовые баллоны"];}
if (courseGo==301 ||courseGo==302 || courseGo==303) {objectfind= ["Вы проверяете багаж, Вам необходимо найти: оружие и  боеприпасы"];}
let superTimer=0;
...
if(pathAria2_1[i]==undefined){rotateArr[i]=false;} else {rotateArr[i]=true;}
...
{
if(pathAria_1[i]==undefined){rotateArr[i]=false;} else {rotateArr[i]=true;}
pathname[i]="./image_rti1/";
pathname1[i]="image_rti1_1/";
pathname2[i]="./image_rti1/";
scopes[i]=10; 
...
superb.insertAdjacentHTML('beforebegin', '<div class="slide"><img  usemap="#id'+random[i]+'_Map"  id="slide'+i+'" BORDER = "0" src='+pathname[i]+files[i]+'+NO.jpg alt=""></div>')
//./img/mva11/
//if(pathAria[i]){superс.insertAdjacentHTML('beforebegin', pathAria[i]);
//if(pathAria_1[i]!=undefined) {superс.insertAdjacentHTML('beforebegin', pathAria_1[i]);}   }
}
...
btnNext.addEventListener("click", function () { if(pauseGO==1){return false;} pauseGO=1; dop=0; region77=0;  
	stopzooom();  //rotate(0);  
	if (scopes[viewSlide]!==0) {alert ("вы не обнаружили запрещённый предмет!");}
...
function click_handler() { 
	...
	btnPrev.addEventListener("click", function () { return false; //- убираем возможность откатить картинку
		
		pauseGO=-1; timeBack=timeGo; stopzooom(); 
 ...
 console.log(err);
  // обработка ошибки
					...
					while (i < things.length) { // выводит 0, затем 1, затем 2
						...
						viewobjecttext.innerHTML = "выявление!"; //"&#128518;";//description[i];
						viewscope.innerHTML =  Math.round((scope/sumscopes)*100) + "%" ; //"&#128518;"; //
