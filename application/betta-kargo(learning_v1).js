...
function popupVideo() {
  ...
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    ...
    navigator.mediaDevices
      ...
      .catch(function (err) {
        alert("Для тестирования необходимо включить камеру");
...
let objectfind = 0;
if (courseGo == 109 || courseGo == 110) {
  objectfind = [
    "Запомните внешний вид опасных объектов и оружия, чтобы в дальнейшем вы смогли обнаружить их в багаже. ",
  ];
}
if (courseGo == 301 || courseGo == 302 || courseGo == 303) {
  objectfind = [
    "Вы проверяете багаж, Вам необходимо найти: оружие и  боеприпасы",
  ];
}
let superTimer = 0;
let viewobjecttext = document.getElementById("viewobjecttext");
let viewscope = document.getElementById("scope");
viewscope.innerHTML = "0%"; //0; "&#128518;"
...
var xx = 0;
var yy = 0;
var levelmva = course*1; //тут была ошибка
var pathname = [];
var pathname1 = [];
...
//random=random.splice(0, 5);
//things=things.splice(0, 5);
//pathAria=pathAria.splice(0, 5);
//description=description.splice(0, 5);
//scopes=scopes.splice(0, 5);
...
for (let i = 0; i < slideAll; i++) {
  ...
  if (b === 0 || i == 0 || b === 3) {
    ...
    if (pathAria_1[i] == undefined) {
      rotateArr[i] = false;
    } else {
      rotateArr[i] = true;
    }
    if (pathAria2_1[i] == undefined) {
      rotateArr[i] = false;
    } else {
      rotateArr[i] = true;
    }
  ...
  if (b === 1 && i != 0) {
    ...
    //answerObj[btb]=scopes[i];
    if (pathAria[i]) {
      superс.insertAdjacentHTML("beforebegin", pathAria[i]);
      if (pathAria_1[i] != undefined) {
        superс.insertAdjacentHTML("beforebegin", pathAria_1[i]);
      }
    }
  }
  ...
  if (b === 2 && i != 0) {
    ...
    pathname[i] = "./image_rti2/";
    pathname1[i] = "image_rti2_1/";
    pathname2[i] = "./image_rti2/";
    scopes[i] = 10;
    ...
    //answerObj[btb]=scopes[i];
    if (pathAria2[i]) {
      superс.insertAdjacentHTML("beforebegin", pathAria2[i]);
      if (pathAria2_1[i] != undefined) {
        superс.insertAdjacentHTML("beforebegin", pathAria2_1[i]);
      }
    }
  }

  superb.insertAdjacentHTML(
    "beforebegin",
    '<div class="slide"><img  usemap="#id' +
      random[i] +
      '_Map"  id="slide' +
      i +
      '" BORDER = "0" src=' +
...
sumscopes = 0;
for (let i = 1; i < things.length; i++) { // i=1 - пропускаем первый слайд
  sumscopes = sumscopes + 10;
  scopes[i] = 10;
  answerAll.push({
    things: things[i],
...
btnNext.addEventListener("click", function () {
  ...
  let object = document.getElementById("imageObject");
  if (object) {
    object.parentNode.removeChild(object);
  }
  viewobject.innerHTML =
    "<img src=" + image_object + 0 + '.png width="250" alt="">';
  pauseGO = 1;
  dop = 0;
  region77 = 0;
  stopzooom(); //rotate(0);
  if (scopes[viewSlide] !== 0) {
    //alert("вы не обнаружили запрещённый предмет!");
  }

  if (viewSlide > 0) {
    document
      .getElementById("slide" + (viewSlide + 0))
      .setAttribute(
        "src",
        pathname[viewSlide] + files[viewSlide + 0] + "+NO.jpg"
      );
  }
...
function click_handler() {
  ...
  if (superTimer < 2) {
    let timer = setTimeout(function tick() {
      ...
      btnPrev.addEventListener("click", function () {
        return false; //- убираем возможность откатить картинку
        ...
        if (viewSlide > 0) {
          document
            .getElementById("slide" + (viewSlide - 1))
            .setAttribute(
              "src",
              pathname[viewSlide] + files[viewSlide - 1] + "+NO.jpg"
            );
        }
      });

      btnPause.addEventListener("click", function () {
        return false; //- убираем возможность остановить картинку
        pauseGO = 0;
        return pauseGO;
      });
      ...
      if (pauseGO == 1) {
        timeGo = timeGo + speed;
        fulltimeGo++;
        /*stopzooom();*/ if (timeGo >= 1000) {
          viewSlide++;
          if (!rotateArr[viewSlide]) {
            btnSecond.style.display = "none";
          } else {
            btnSecond.style.display = "flex";
          }
          console.log(rotateArr[viewSlide]);
          timeGo = 0;
          nameSlide++;
          pauseGO = 0;
          sumClicksOneImage = 0;
      ...
      if ((viewSlide === slideAll - 1 && timeGo >= 700) || min == "15") {
        ...
        if (clicksover < 0) {
          scopess = Math.round((scope / sumscopes) * 100) + 5 * clicksover;
        } else {
          scopess = Math.round((scope / sumscopes) * 100);
        }
        ...
        function demo1() {
          ...
          myVar =
            ...
            " " +
            encodeURIComponent(timefortest) +
            " " +
            encodeURIComponent(scopess) +
            " " +
            encodeURIComponent(courseGo) +
            " " +
            encodeURIComponent(clicks) +
            " " +
            encodeURIComponent(secondsAll) +
            " " +
            encodeURIComponent(sumscopes / 10) +
            " " +
            encodeURIComponent(stroka) +
            " " +
            encodeURIComponent(scopess1);
      ...
      if (pauseGO == -1 && timeBack - timeGo < 500 && viewSlide >= 0) {
        timeGo--;
        if (timeGo < 0) {
          viewSlide--;
          timeGo = 1000;
      ...
      return (viewobjectslider =
        '<img src="./img_object/' + 0 + '.png"  alt=""> ');
      return dop;
      return pauseGO;
      return region77;
      return viewSlide;
  ...
  btnSpeed.addEventListener("click", function () {
    if (speed == 1) {
      speed = 2;
      document.getElementById("speed").innerHTML = "скорость Х1";
    } else {
      speed = 1;
  ...
  btnSensetiv.addEventListener("click", function () {
    dop = "S";
    region77 = 0;
    pauseGO = 0;
    document
      .getElementById("slide" + viewSlide)
      .setAttribute(
        "src",
        pathname[viewSlide] + files[viewSlide + 0] + "+" + dop + ".jpg"
      ); // 1=S чуствительность
    region77 == 1;
  });
  ...
  btnNonOrganic.addEventListener("click", function () {
    dop = "OS";
    region77 = 0;
    pauseGO = 0;
    document
  ...
  btnOrganic.addEventListener("click", function () {
    dop = "OO";
    region77 = 0;
    pauseGO = 0;
    document
  ...
  btnInverse.addEventListener("click", function () {
    dop = "NE";
    region77 = 0;
    console.log(dop, region77, viewSlide);
    pauseGO = 0;
    document
      .getElementById("slide" + (viewSlide + 0))
      .setAttribute(
  ...
  btnSecond.addEventListener("click", function () {
    ...
    dop = "NO";
    region77 = 0; //console.log(dop,region77,viewSlide);
    pauseGO = 0; //stopzooom();
    rotate(1);
    document
      .getElementById("slide" + (viewSlide + 0))
...
function rotate(a) {
  ...
  if (a == 1 && ss == -1) {
    pathname[viewSlide] = pathname1[viewSlide];
    ...
    btnSecond.innerHTML = "&#8635";
  } else {
    pathname[viewSlide] = pathname2[viewSlide];
    btnSecond.innerHTML = "&#8634";
...
function hiddens(a) {
  ...
  for (pi = 0; pi < point.length; pi++)
    if (point[pi].style.visibility == "hidden") {
      point[pi].style.visibility = "visible";
    } else {
      point[pi].style.visibility = "hidden";
    }
}
...
function superClick(x, y) {
  document.querySelector("#slide").addEventListener("click", function (e) {
    ...
    if (pauseGO !== 0 || zooming) {
      return false;
    }
    id_t = e.target.id; // Получили ID, т.к. в e.target содержится элемент по которому кликнули

    if (id_t.substr(0, 4) == "blah") {
      console.log(id_t.substr(0, 4));
      var child = document.getElementById(id_t);
        ...
        for (let key in searchRegion) {
          if (key == id_t) {
            ...
            if (w == 1) {
              ...
              scope = scope - cloneScopes[j2];
              scopes[j2] = cloneScopes[j2];
              //alert(searchRegion[key]);
...
function selectRegion(ctx, region) {
  ...
  sumClicksOneImage++;
  answerAll[viewSlide].click = sumClicksOneImage;
  var slideClick = ctx.parentElement.name.slice(2, -4);
  superClick1(-15, 510);
  ...
  while (i < things.length) {
    ...
    if (region == things[i]) {
      ...
      viewobjecttext.innerHTML = description[i]; //"&#128518;";//"выявление!";
      viewscope.innerHTML = Math.round((scope / sumscopes) * 100) + "%"; //"&#128518;"; //image_object
      console.log(region);
      viewobject.innerHTML =
        "<img id='image_object' src=" +
        image_object +
        region +
        '.jpg width="250" alt=""> ';
      scopes[i] = 0;
  ...
  regiomImg = "image_rti/" + files[slideClick] + "+NO.jpg";
  sessionStorage.setItem("regiomImg", regiomImg);
  return (region77 = 1);
}
...
function change() {
  ...
  try {
    a = document.getElementById("image_object").src;
    tempPauseGO = pauseGO;
    pauseGO = 0;
    var mimg = document.createElement("img");
    mimg.src = a;
    mimg.setAttribute("id", "imageObject");
    mimg.style.position = "absolute";
    mimg.style.top = 5 + "px";
    mimg.style.left = 5 + "px";
    mimg.style.width = 1000 + "px";
    mimg.style.height = 640 + "px";
    mimg.style.zIndex = 10001;
    document.getElementsByTagName("body")[0].appendChild(mimg);

    let regiomImg = sessionStorage.getItem("regiomImg");
    if (regiomImg) {
      viewobject.innerHTML = '<img src="' + regiomImg + '"width="250" alt=""/>';
      console.log("wq1111", regiomImg);
      changes = true;
      return changes;
    } else {
      viewobject.innerHTML =
        ...
        pathname +
        files[viewSlide + 0] +
        '+NO.jpg"width="250" alt=""/>';
      console.log("wqqqw", "slide");
      changes = true;
      return changes;
    }
  } catch (err) {
    let mimgs = document.getElementById("imageObject");
    try {
      let mimgd = document.getElementById("imageObject").src;
      setTimeout(() => mimgs.remove(), 10);
      pauseGO = tempPauseGO;
      viewobject.innerHTML =
        '<img src="' + mimgd + '"width="250" id="image_object" alt=""/>';
      changes = false;
      return changes;
    } catch (err) {}
  }
...
async function zooom() {
  ...
  btnZoom.addEventListener(mobileDown, function () {
    ...
    blinks = setInterval(blinkin, 250);
  ...
  function blinkin() {
    if (document.getElementById("exit1").style.fontWeight == "900") {
      document.getElementById("exit1").style.fontWeight = "100";
    } else {
      document.getElementById("exit1").style.fontWeight = "900";
    }
  }
}
...
function stopzooom() {
  ...
  for (let i = 0, length = arrayzoom.length; i < length; i++) {
    if (newarrai[i] == 1) {
      newarrai[i] = 0;
      arrayzoom[i].style.visibility = "visible";
    }
  }
...
function testTimer(startTime) {
  ...
  if (startTime >= 0) {
    ...
    document.getElementById("time").innerHTML =
      "<span>Осталось времени- 00 мин 00 секунд</span>";
...
function loadDoc(lessons, course) {
  ...
  console.log(lessons+" dddd "+course);
  return files;
}
...
if (getCookie("id_xtvs") == null) {
  document.location = "https://training.aeromash.ru/xtvs/";
}
