...
$query = mysqli_query($link,"SELECT user_name FROM users_act WHERE user_login='".$_COOKIE['id']."' LIMIT 1");
$data = mysqli_fetch_assoc($query);
if(!$_COOKIE['id']||$_COOKIE['id'] !== "boss"){header("Location: index.php");}
...
div.onclick = function (e) {
 ...
 window.onload = function () { 
  ...
  if (obj.id[9]){a=obj.id[9]} else {a=0;}
  let lesson = Number(obj.id[8])+Number(a);
...
function runLesson(course, courseGo) {
	...
	let nowLesson = "lessons"+(courseGo-100*course);
	
	setCookie("nowLesson", nowLesson);
	setCookie("courseGo", courseGo);
	setCookie("course", course);
	
	let lesson = courseGo+"l";
	lesson=lesson[2];
	let nameId="times787";
	if(getCookie(nameId)=="1"&&getCookie("courseGo")=="305"){
...
function summa(post) {
    ...
    xmlhttp.open('POST', 'answer.php', true); // Открываем асинхронное соединение
...
<div id="textname"><?php echo $data['user_name']." (".$_COOKIE['id'].")"; ?></div>
<div id="welcome"> 

    <h1>Добро пожаловать на <br> x-ray TV simulator</h1> <h2>(симулятор рентгено-телевизионной установки)</h2>  <h1>выберите что <br> вам необходимо </h1>
	
   
           <label for="date" class="control-label">Начало</label>
		   <label for="date" class="control-label">Конец</label><br>
   ...
   <div id="imput"> 
        ...
        <div class="course1" id="course1">Посмотреть результаты<br> по изучению теневых изображений</div>  
        <div class="course1" id="course2">Скачать результаты всех работников<br> по изучению теневых изображений</div> 
		<div class="course1" id="course3">Добавить или удалить <br> работника</div>  		
